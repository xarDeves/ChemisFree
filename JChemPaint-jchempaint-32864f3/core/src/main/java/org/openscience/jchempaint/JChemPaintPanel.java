...
import org.openscience.jchempaint.controller.*;
...
public class JChemPaintPanel extends AbstractJChemPaintPanel implements
        ...
        public void windowClosing(WindowEvent e) {
            ...
            if (JOptionPane.CANCEL_OPTION != clear) {
                for (int i = 0; i < instances.size(); i++) {
                    if (instances.get(i).getTopLevelContainer() == e
                            .getSource()) {
                        instances.remove(i);
                        break;
                    }
    ...
    public void selectionChanged() {
        ...
        if (this.getRenderPanel().getRenderer().getRenderer2DModel().getSelection() != null) {
            ...
            enOrDisableMenus(atomMenu, selection.getConnectedAtomContainer() != null && selection.getConnectedAtomContainer().getAtomCount() > 0);

            enOrDisableMenus(bondMenu, selection.getConnectedAtomContainer() != null && selection.getConnectedAtomContainer().getBondCount() > 0);
    ...
    public void modeChanged(IControllerModule newActiveModule) {
        ...
        if (JCPToolBar.getToolbarResourceString("lefttoolbar", getGuistring()).contains(newActiveModule.getID()) && !newActiveModule.getID().equals("reactionArrow")) {
        ...
        if (JCPToolBar.getToolbarResourceString("lowertoolbar", getGuistring()).contains(newActiveModule.getID())) {
